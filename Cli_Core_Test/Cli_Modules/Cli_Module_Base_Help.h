...
public:
    ...
    static int Help_Cli_Modules_Get_Len(int user_privilege, Cli_Modules &Cli_Modules,
            string Level, bool is_verbose,
            string filter, Str_Filter_Abstract &str_filter) {
        int len_max = 0;
        ...
        for (int module = 0; module < Cli_Modules.Get_Size(); module++) {
            ...
            for (int cmd = 0; cmd < climodule_ptr->Module_Cmd_List.size(); cmd++) {
                ...
                if (
                        ...
                        && str_filter.Is_Match(filter, cmd_ptr->Items[0]->Text_Get())
                        ) {
                    int len = 0;
                    if (Level.size() == 0) {
    ...
    static void Help_Cli_Modules_Print(int user_privilege, Cli_Modules &CliModules,
            string Level, bool is_verbose,
            string filter, Str_Filter_Abstract &str_filter,
            int len_max, int &count,
            Cli_Output_Abstract &Cli_Output) {

        Cli_Output.Output_NewLine();

        for (int module = 0; module < CliModules.Get_Size(); module++) {
            Cli_Module *climodule = CliModules.Get(module);
            for (int cmd = 0; cmd < climodule->Module_Cmd_List.size(); cmd++) {
                Cli_Cmd *cmd_ptr = climodule->Module_Cmd_List[cmd];
                string s;
                stringstream s_str1;
                stringstream s_str2;
                if (Level.size() == 0) {
    ...
    void help(string Level, bool is_verbose, string filter, Cli_Output_Abstract &Cli_Output) {
        int count = 0;
        int len_max = 0;
        int len;

        len = Help_Cli_Modules_Get_Len(User_Privilege, Modules, Level, is_verbose, filter, Help_Str_Filter);
        if (len_max < len)
            len_max = len;

        Help_Cli_Modules_Print(User_Privilege, Modules, Level,
                is_verbose, filter, Help_Str_Filter,
                len_max, count,
                Cli_Output);

        if (count == 0) {
            Cli_Output.Output_Str("\"");
            Cli_Output.Output_Str(filter);
    ...
    virtual bool Execute(Cli_Cmd_ID cmd_id, Cli_Cmd *cmd, vector<Level_Description> &Levels, bool is_debug) {
        ...
        switch (cmd_id) {
                ...
                if (is_debug) return true;
            {
                ...
                string filter = "*";
                help(level.Level, is_verbose, filter, Cli_Output);
            }
                return true;
            ...
            {
                bool is_verbose = false;
                string filter = cmd->Items[1]->Value_Str;
                help(level.Level, is_verbose, filter, Cli_Output);
            }
                return true;
