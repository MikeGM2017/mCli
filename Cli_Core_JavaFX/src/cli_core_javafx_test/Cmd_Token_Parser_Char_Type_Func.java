 ...
 * @author mike
 */
public class Cmd_Token_Parser_Char_Type_Func {

    public static Cmd_Token_Parser_Char_Type Char_Type_Get(char c) {
        if (c >= 'a' && c <= 'z') {
            return Cmd_Token_Parser_Char_Type.CMD_TOKEN_PARSER_CHAR_TYPE_ALPHA;
        }
        if (c >= 'A' && c <= 'Z') {
            return Cmd_Token_Parser_Char_Type.CMD_TOKEN_PARSER_CHAR_TYPE_ALPHA;
        }
        ...
        if (c >= '0' && c <= '9') {
            return Cmd_Token_Parser_Char_Type.CMD_TOKEN_PARSER_CHAR_TYPE_DIGIT;
        }
        switch (c) {
            case ' ':
            case '\t':
        ...
        if (c == '.') {
            return Cmd_Token_Parser_Char_Type.CMD_TOKEN_PARSER_CHAR_TYPE_POINT;
        }
