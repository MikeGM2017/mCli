 ...
 * @author mike
 */
public class Cmd_Item_Word_Range extends Cmd_Item_Word {

    public List<String> Words;

    Cmd_Item_Word_Range(String text, String help, List<String> words) {
        super(text, help);
        Words = words;
        Type = "Word_Range";
    }
    ...
    public List<String> Incomplete_Tail_List_Get(String s) {
        ...
        for (int i = 0; i < Words.size(); i++) {
            if (Words.get(i).substring(0, s.length()).equals(s)) {
                //tail_list.add(Words.get(i).substring(0, s.length()));
                //tail_list.add(Words.get(i).substring(s.length()+1, Words.get(i).length()));
