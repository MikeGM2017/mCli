...
static const Test_Item TestVector_01[] = {
    ...
    Test_Item("\" Str Str \"", " Str Str ", CMD_ITEM_OK_STR_WITHOUT_COMMAS),
    Test_Item("\" ", "\" ", CMD_ITEM_INCOMPLETE_STR),

    // Escaped:
    Test_Item("a\\\"", "a\\\"", CMD_ITEM_ERROR),
    ...
    Test_Item("\\\"a\\\"", "\"a\"", CMD_ITEM_OK),
    Test_Item("\\\"a", "\\\"a", CMD_ITEM_INCOMPLETE_STR),
    ...
    Test_Item("\'\"\"\'", "\"\"", CMD_ITEM_OK_STR_WITHOUT_COMMAS),
    // Escaped
    Test_Item("\'\\\"\\\"\'", "\"\"", CMD_ITEM_OK_STR_WITHOUT_COMMAS), // @Warning!!! Case: Escaped, but '\'' - not escaped!!!
};
...
void test1() {
    ...
    for (int i = 0; i < test_vector_size; i++) {
        ...
        if (res != TestVector[i].Res || v.Value_Str != TestVector[i].Res_Str) {
            std::cout << "%TEST_FAILED% time=0 testname=test1 (Test_Cmd_Item_Str_Esc) message="
                    << " res=" << Cmd_Item_Valid_Result_Func::To_String(res)
